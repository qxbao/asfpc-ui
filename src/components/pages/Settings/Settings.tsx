        ...
        <StatCard
          ...
          value={
            isLoading
              ? loadingIcon
              : totalSettings -
                Object.keys(facebookSettings).length -
                Object.keys(geminiSettings).length
          }
...
function SettingsForm() {
  ...
  const categorizeSettings = () => {
    ...
    Object.keys(flattenedSettings).forEach((key) => {
      ...
      if (upperKey.startsWith("FACEBOOK")) {
        ...
        upperKey.startsWith("CONCURRENCY") ||
        upperKey.startsWith("LIMIT")
      ) {
  ...
  const renderSettingsByCategory = (
    title: string,
    categoryKey: keyof ReturnType<typeof categorizeSettings>,
    color: string,
  ) => {
              ...
              {isBooleanField(settingKey) ? (
                ...
                >
                  <Typography variant="body2">
                    {formatLabel(
                      settingKey
                        ? settingKey.split("_").slice(0, -1).join("_")
                        : "",
                    )}
                  </Typography>
